...
$CONTENT"
    ...
    if timeout --kill-after=30s 5400s claude --dangerously-skip-permissions "$PROMPT"; then
        # Move to testing (skip if Claude already moved it via Gate 4)
        if [ -f "$FILE" ]; then
            ...
            echo ""
            echo "NOTE: $NAME already moved (Claude handled Gate 4)"
        fi
